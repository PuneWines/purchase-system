...
async function run() {
  ...
  while (hasMore) {
    const { data: pageData, error: pageError } = await supabase
      ...
      .order("id", { ascending: false }) // stable sorting
  ...
  console.log('Total items fetched with stable sorting:', allIndentItems.length);
  ...
  console.log('Duplicate IDs found with stable sorting:', duplicateIds.length);
}
